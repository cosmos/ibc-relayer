...
var iftAddress = flag.String(
	"ift-address",
	"",
	"address of the IFT token contract on the source chain (ift only)",
)

func iftTransfer(ctx context.Context) error {
	lmt.Logger(ctx).Info("config path", zap.String("path", *cfgPath))
	lmt.Logger(ctx).Info("ift address", zap.String("ift_address", *iftAddress))
	...
	if sourceChain.EVM == nil {
		return errors.New("ift transfers require an evm source chain")
	}
	...
	if !common.IsHexAddress(*iftAddress) {
		return fmt.Errorf("ift-address %s is not a valid hex address", *iftAddress)
	}
	...
	lmt.Logger(ctx).Info(
		fmt.Sprintf("sending ift transfer from %s to %s", *sourceChainID, *destChainID),
		zap.String("ift_address", *iftAddress),
		zap.String("receiver", *receiver),
		zap.String("amount", amt.String()),
	)

	sendTxHash, err := sourceClient.IFTTransfer(ctx, *iftAddress, *sourceClientID, *receiver, amt, *timeout)
	if err != nil {
		return fmt.Errorf("sending ift transfer from %s to %s: %w", *sourceChainID, *destChainID, err)
	}

	lmt.Logger(ctx).Info(
		fmt.Sprintf("successfully sent ift transfer from %s to %s", *sourceChainID, *destChainID),
